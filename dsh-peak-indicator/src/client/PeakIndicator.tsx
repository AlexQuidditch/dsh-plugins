...
const PEAK_WINDOWS: ReadonlyArray<readonly [number, number]> = [[60, 240], [360, 600]]
...
function isPeak(day: number, minutes: number): boolean {
...
function minutesToNextChange(day: number, minutes: number): number {
...
function currentState(): PeakState {
  const now = new Date()
...
function localWindowLabel(): string {
  const now = new Date()
  const y = now.getFullYear()
  const mo = now.getMonth()
  const da = now.getDate()
  const local = (min: number): string =>
    format(new Date(Date.UTC(y, mo, da, Math.floor(min / 60), min % 60)), 'HH:mm')
  return PEAK_WINDOWS
...
export function PeakIndicator() {
  const [state, setState] = useState<PeakState>(() => currentState())
  ...
  return (
    <span className={state.peak ? `${styles.ind} ${styles.on}` : styles.ind} title={localWindowLabel()}>
