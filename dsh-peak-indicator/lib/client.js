window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/Users/alex_quidditch/projects/dsh-plugins/dsh-peak-indicator/src/client/PeakIndicator.module.css.mjs
		const css = ".qLadUa_ind{white-space:nowrap;border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);cursor:default;border-radius:999px;align-items:center;gap:8px;padding:5px 12px;font-size:12px;line-height:1.5;transition:box-shadow .4s,background .4s,color .4s,border-color .4s;display:inline-flex}.qLadUa_ind .qLadUa_dot{background:var(--dsw-alias-state-success-primary);border-radius:50%;width:9px;height:9px;transition:background .4s,box-shadow .4s}.qLadUa_on{color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-state-error-primary);background:color-mix(in srgb, var(--dsw-alias-state-error-primary) 16%, var(--dsw-alias-bg-layer-1));animation:2.2s ease-in-out infinite qLadUa_dskPeakPulse}.qLadUa_on .qLadUa_dot{background:var(--dsw-alias-state-error-primary);box-shadow:0 0 8px 1px var(--dsw-alias-state-error-primary)}@keyframes qLadUa_dskPeakPulse{0%,to{box-shadow:0 0 6px 1px var(--dsw-alias-state-error-primary)}50%{box-shadow:0 0 16px 5px var(--dsw-alias-state-error-primary)}}";
		const tagId = "dsh-peak-indicator/PeakIndicator.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-peak-indicator";
			tag.dataset.pluginCss = tagId;
			tag.textContent = css;
			document.head.appendChild(tag);
		}
		var PeakIndicator_module_css_default = {
			"dot": "qLadUa_dot",
			"dskPeakPulse": "qLadUa_dskPeakPulse",
			"ind": "qLadUa_ind",
			"on": "qLadUa_on"
		};
		//#endregion
		//#region src/client/PeakIndicator.tsx
		/**
		...
		*/
		...
		function minutesToNextChange(day, minutes) {
			...
			for (let d = 0; d <= 8; d += 1) {
				...
				for (const window of PEAK_WINDOWS) for (const boundary of window) {
					const delta = (day + d) * DAY_MIN + boundary - nowAbs;
		...
		function currentState() {
			...
			return {
				peak,
				untilChange,
				text: peak ? `конец пика через ${formatRemaining(untilChange)}` : `пик через ${formatRemaining(untilChange)}`
			};
		}
		...
		function localWindowLabel() {
			...
			return PEAK_WINDOWS.map(([start, end]) => `${local(start)}–${local(end)}`).join(" и ");
		}
		function PeakIndicator() {
			const [state, setState] = (0, react.useState)(() => currentState());
			(0, react.useEffect)(() => {
				const id = setInterval(() => setState(currentState()), 3e4);
				return () => clearInterval(id);
			}, []);
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
				className: state.peak ? `${PeakIndicator_module_css_default.ind} ${PeakIndicator_module_css_default.on}` : PeakIndicator_module_css_default.ind,
				title: localWindowLabel(),
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { className: PeakIndicator_module_css_default.dot }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: state.text })]
			});
		}
		//#endregion
