...
import type { PromptContentPart, SessionId } from '@deepseek-ai/dsh-client-connection/client'

import { HelloAction } from './HelloAction.tsx'
import { HELLO_LINE, type HelloInjected } from './slots.ts'
...
export function apply(ctx: ClientContext): void {
  ctx.slots.inject('conversation.chat.assistant-actions', () => ctx.slots.register({
    ...
    id: 'hello',
    order: 10,
    inject: (sessionId: SessionId): HelloInjected => ({
      runHello: async () => {
        ...
        if (session === undefined) throw new Error(`no session for ${sessionId}`)
        if (line.startsWith('/')) {
        ...
        const content: PromptContentPart[] = [{ type: 'text', text: line }]
