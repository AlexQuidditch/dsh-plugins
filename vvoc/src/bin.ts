...
import { analyticsTable, type GroupBy } from './analytics.js'
...
async function commandLint(rest: string[], flags: Set<string>): Promise<number> {
  ...
  if (explicit.length === 0) {
    ...
    for (const packageName of readdirSync(root)) {
      ...
      if (!includeArchive && packageName.startsWith('archive')) continue
...
function commandAnalytics(rest: string[], flags: Set<string>): number {
  const groupByFlag = rest.indexOf('--group-by')
  const groupBy = (groupByFlag >= 0 ? rest[groupByFlag + 1] : 'day') as GroupBy
  ...
  const fmt = (rate: number | undefined): string => rate === undefined ? 'n/a' : `${(rate * 100).toFixed(1)}%`
  if (json) {
...
function commandRole(rest: string[], flags: Set<string>): number {
  const sub = rest[0]
  ...
  if (sub === 'list') {
    ...
    for (const row of rows) console.log(`${row.role.padEnd(16)} ${row.model ?? ''}${row.scope === null ? '' : `  [${row.scope}]`}`)
    return 0
  }
  if (sub === 'set') {
    const role = rest[1]
    const model = rest[2]
    if (role === undefined || model === undefined) { console.error('использование: vvoc role set <role> <provider/model> [--global]'); return 1 }
    const roleError = validateRoleId(role)
    if (roleError !== undefined) { console.error(roleError); return 1 }
    const modelError = validateModel(model)
    if (modelError !== undefined) { console.error(modelError); return 1 }
  ...
  if (sub === 'unset') {
    const role = rest[1]
    if (role === undefined) { console.error('использование: vvoc role unset <role> [--global]'); return 1 }
    if (!unsetRole(role, globalScope)) { console.error(`роль ${role} не задана в выбранной области`); return 1 }
    console.log(`роль ${role} удалена (${globalScope ? 'global' : 'project'})`)
    return 0
  }
  console.error('использование: vvoc role list|set|unset'); return 1
}

function commandPreset(rest: string[], flags: Set<string>): number {
  const globalScope = hasFlag(flags, '--global')
  if (rest.length === 0 || rest[0] === 'list') {
    const rows = listPresets()
    if (rows.length === 0) { console.log('пресеты не заданы (в vvoc.json: {"presets": {"<name>": {"reviewer": "provider/model", ...}}}'); return 0 }
    for (const row of rows) {
  ...
  if (rest[0] === 'show') {
    const name = rest[1]
    if (name === undefined) { console.error('использование: vvoc preset show <name>'); return 1 }
    const row = listPresets().find((item) => item.name === name)
    if (row === undefined) { console.error(`пресет ${name} не найден`); return 1 }
    for (const [role, model] of Object.entries(row.roles)) console.log(`${role.padEnd(16)} ${model}`)
    return 0
  }
  const error = applyPreset(rest[0], globalScope)
  if (error !== undefined) { console.error(error); return 1 }
  ...
  switch (command) {
    ...
    case 'sync': {
      try {
        ...
        if (result.backup !== undefined) console.log(`старая копия: ${result.backup}`)
        ...
        if (command === 'sync') {
          ...
          for (const skill of bad) console.log(`  ${skill.name}: ${skill.reason ?? 'проблема'}`)
    ...
    case 'analytics': {
      if (rest[0] === 'cache-hit-rate' || rest[0] === undefined) return commandAnalytics(rest, flags)
      console.error(`неизвестная подкоманда analytics: ${rest[0]}`); return 1
    }
    case 'role': return commandRole(rest, flags)
    case 'preset': return commandPreset(rest, flags)
    case '--help':
    case 'help':
    case '': console.log(USAGE); return 0
    default: console.error(`неизвестная команда: ${command}`); console.error(USAGE); return 1
  }
}
