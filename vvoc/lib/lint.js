...
function countDescendants(node, path) {
    let current = [node];
...
export function lintXml(text, kind, options = {}) {
    ...
    if (root.name !== kind)
        verdict.errors.push({ rule: 'root', message: `root element must be <${kind}>, found <${root.name}>` });
