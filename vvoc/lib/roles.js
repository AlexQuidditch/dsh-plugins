...
const MODEL_RE = /^[^\s/]+\/[^\s/]+$/;
export function configPath(globalScope) {
...
export function effectiveConfig() {
    ...
    if (existsSync(globalConfigPath()))
        sources.push({ path: globalConfigPath(), scope: 'global' });
    const project = readConfig(projectConfigPath());
    if (existsSync(projectConfigPath()))
        sources.push({ path: projectConfigPath(), scope: 'project' });
    return { config: { roles: { ...global.roles, ...project.roles }, presets: { ...global.presets, ...project.presets } }, sources };
}
export function validateRoleId(role) {
    if (!ROLE_ID_RE.test(role))
        return `role id must match ${ROLE_ID_RE}, got "${role}"`;
    return undefined;
}
export function validateModel(model) {
    if (!MODEL_RE.test(model))
        return `model must look like "provider/model" (no spaces, one slash), got "${model}"`;
    return undefined;
}
...
function scopeOf(role) {
    const project = readConfig(projectConfigPath());
    if (project.roles?.[role] !== undefined)
        return 'project';
    const global = readConfig(globalConfigPath());
    if (global.roles?.[role] !== undefined)
        return 'global';
    return null;
}
export function setRole(role, model, globalScope) {
    const path = configPath(globalScope);
    const config = readConfig(path);
    config.roles = { ...config.roles, [role]: model };
    writeConfig(path, config);
}
export function unsetRole(role, globalScope) {
    const path = configPath(globalScope);
    const config = readConfig(path);
...
export function applyPreset(name, globalScope) {
    const path = configPath(globalScope);
    const config = readConfig(path);
    const preset = config.presets?.[name];
    if (preset === undefined)
        return `preset "${name}" not found in ${path}`;
    config.roles = { ...preset };
    writeConfig(path, config);
    return undefined;
}
